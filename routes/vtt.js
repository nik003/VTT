...
console.log(path.resolve(__dirname, '../model/stops.json'));
const fs = require('fs');
let stopsFile = fs.readFileSync(path.resolve(__dirname, '../model/stops.json'));
let stops = JSON.parse(stopsFile);
...
router.get('/id/:id', (req, res) => {
  for (var stop in stops.stops) {
    console.log(stops.stops[stop]);
    if (stops.stops[stop].id == req.params.id) {
      cookieHandling(req, res, (accessToken) => {
        stopData.getStops2(accessToken, stop.stopID).then(stopdata.queryVttApi).then((data) => {
          objData = divideData(req, data);
          res.render('index.html', {
            curtime: curTime,
            txtSize: objData.textSize,
            stop: data.stop,
            board: objData.list
          })
        });
